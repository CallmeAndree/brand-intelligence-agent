...
function Kpi({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="rounded-card border border-ink/10 bg-white/60 p-4">
      <div className="text-label uppercase text-ink/50">{label}</div>
      <div className={`mt-1 text-title-lg ${accent ?? "text-ink"}`}>{value}</div>
    </div>
  );
}
...
export default function KpiStrip() {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
      ...
      <Kpi label="Avg Severity" value={k ? String(k.avg_severity) : dash} />
      <Kpi
        label="Critical (≥7)"
