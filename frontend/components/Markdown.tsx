...
export default function Markdown({ children }: { children: string }) {
  return (
    <div className="space-y-2 text-[14px] leading-relaxed text-ink/80">
      <ReactMarkdown
        remarkPlugins={[remarkGfm]}
        components={{
          h1: (p) => <h1 className="text-base font-semibold text-ink" {...p} />,
          h2: (p) => <h2 className="mt-3 text-[15px] font-semibold text-ink" {...p} />,
          h3: (p) => <h3 className="mt-2 text-sm font-semibold text-ink/90" {...p} />,
          ...
          a: (p) => <a className="text-feature-pink underline" target="_blank" rel="noreferrer" {...p} />,
          code: (p) => <code className="rounded bg-ink/5 px-1 py-0.5 text-[13px]" {...p} />,
          blockquote: (p) => (
